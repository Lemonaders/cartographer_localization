    ...
    for (size_t i = 0; i < matcher_size; i++) 
    {
        ...
        task->SetWorkItem([i, &filtered_point_cloud, &matchers, &score_set, &pose_set, cutoff, &matched_counter, &has_matched, &task_durations, &timing_mutex] {
          ...
          auto start_time = std::chrono::steady_clock::now();
            ...
            try {
                if (matchers[i]->OptimizedMatchFullSubmap(filtered_point_cloud, cutoff, &score, &pose_estimate)) 
                {
                    score_set[i] = score;
...
//                 << "---------------------------------------------------\n"
//                 << "[GlobalPositioningTest] Given_initial_pose:" << Given_initial_pose.DebugString() << ".\n"
//                 << "[GlobalPositioningTest] Given_initial_pose.inverse():" << Given_initial_pose.inverse().DebugString() << ".\n"
//                 << "---------------------------------------------------\n";
...
//     LOG(ERROR) << "\ninit_pose_local_list  11:" <<global_submap_pose.inverse() * cartographer::transform::Project2D(Given_initial_pose);
//     LOG(ERROR) << "\nglobal_submap_pose.inverse()" <<global_submap_pose.inverse();
//     LOG(ERROR) << "\ncartographer::transform::Project2D(Given_initial_pose)" << cartographer::transform::Project2D(Given_initial_pose);
//     const double distance = 
//       (global_submap_pose.translation() - 
//         Given_initial_pose.translation().head<2>()).norm();
...
//     if (distance <= search_submap_range) {   
//       nearby_submaps.push_back(id);
//       LOG(ERROR) << "\ninit_pose_local_list  222222222222:"  <<global_submap_pose.inverse() * cartographer::transform::Project2D(Given_initial_pose);
//     } 
//   }
...
//             LOG(INFO) << "Processing2 matcher index: " << i;
//             try {
//               LOG(ERROR) << "\ninit_pose_local_list  use:" << i << "<<echo" <<init_pose_local_list[i];
//                 if (matchers[i]->Match(init_pose_local_list[i], filtered_point_cloud, cutoff, &score, &pose_estimate)) 
//                 {
//                     LOG(ERROR) << "\nscore  use:"<<i<<"echo:" << score ;
//                     score_set[i] = score;
//                     LOG(ERROR) << "\npose_estimate  use:"<<i<<"echo:" << pose_estimate ;
//                     pose_set[i] = pose_estimate;
//                     has_matched = true;
//                 } else {
//                   LOG(INFO) << "match failed. ";
...
//                                                              << best_pose_estimate->rotation().angle() <<"\n"
//             << "[GlobalPositioningTest] best_score:" <<  *best_score << "\n"
//             << "The distance between best_pose_estimate and Given_initial_pose:"
//             << (pose_set[max_position].translation() - Given_initial_pose.translation().head<2>()).norm()
//             << "\n---------------------------------------------------\n";
//   return true;
//   }
...
//     {
//         auto task = absl::make_unique<common::Task>();
//         task->SetWorkItem([i, &filtered_point_cloud, &matchers, &score_set, &pose_set, cutoff, &matched_counter, &has_matched, &task_durations, &timing_mutex, Given_initial_pose] {
//           //计时开始                                             
//           auto start_time = std::chrono::steady_clock::now();
...
//             LOG(INFO) << "Processing2 matcher index: " << i;
//             try {
//                 if (matchers[i]->Match(cartographer::transform::Project2D(Given_initial_pose), filtered_point_cloud, cutoff, &score, &pose_estimate)) 
//                 {
//                     score_set[i] = score;
//                     pose_set[i] = pose_estimate;
//                     has_matched = true;
...
// struct Array2iHash {                                                        //！！！！！！！！！！！！！！！！！！！！！
//   std::size_t operator()(const Eigen::Array2i& key) const noexcept {
//     return std::hash<int>()(key.x()) ^ (std::hash<int>()(key.y()) << 1);
//   }
// };

// struct Array2iEqual {
//   bool operator()(const Eigen::Array2i& lhs, const Eigen::Array2i& rhs) const noexcept {
...
//         std::unordered_map<Eigen::Array2i, float, Array2iHash, Array2iEqual> log_odds_cache;                    //！！！！！！！！！！！！！！！！！！！！！
//         for (const auto& id : nearby_submaps) {
...
//                 float prob = grid->GetProbability(cell_index);                //！！！！！！！！！！！！！！！！！！！！！
//                 log_odds_cache[dst_index] += prob;                            //！！！！！！！！！！！！！！！！！！！！！
//                     }
//             }
//           }
//         }
//         for (const auto& kv : log_odds_cache) {                              //！！！！！！！！！！！！！！！！！！！！！
//             const Eigen::Array2i& idx = kv.first;
//             const float& prob = kv.second;
//             fused_grid->SetProbability(idx, prob);
//         }                                                                   //！！！！！！！！！！！！！！！！！！！！！
...
//BETA-3-Search scope self expanding and Fusion submap patch
// bool PoseGraph2D::GlobalPositioningTest(cartographer::transform::Rigid3d Given_initial_pose,
//                                         const cartographer::sensor::TimedPointCloud& laser_point_cloud,
//                                         float cutoff, 
//                                         transform::Rigid2d* best_pose_estimate, 
...
//   cartographer::sensor::PointCloud filtered_point_cloud;
//   //子图扫描范围
//   double search_submap_range = 5.0; //m
//   double if_search_submap_range = search_submap_range; //m
//   constexpr int    MaxExpand   = 4 ;
//   constexpr double ExpandRatio = 1.5;
//   std::vector<cartographer::mapping::SubmapId> nearby_submaps;
//   std::vector<cartographer::mapping::SubmapId> matched_submaps;
//   std::vector<transform::Rigid2d> init_pose_local_list;

//   std::vector<float> score_set_vector;
//   std::vector<transform::Rigid2d> pose_set_vector;
//   std::vector<const cartographer::mapping::Grid2D*> submaps_vector;
//   std::atomic_bool has_matched{false};   
//   std::mutex submaps_vector_mutex;
...
//   for (int attempt = 0; attempt < MaxExpand; ++attempt) {
//       nearby_submaps.clear();
//       init_pose_local_list.clear();
//       score_set_vector.clear();
//       pose_set_vector.clear();
//       submaps_vector.clear();
//       if (search_submap_range != if_search_submap_range){
//               LOG(WARNING)  << "\n*****************************************************\n"
//                       << "No matching submap found, expand range to " << search_submap_range;}
//       // 基于初始位姿筛选子图
//       for (const auto& submap_id_data : data_.submap_data) {
//         const SubmapId& id = submap_id_data.id;

//         // if (std::find(matched_submaps.begin(), matched_submaps.end(), id) != matched_submaps.end()) {
//         //   continue;
//         // }
//         if (id.trajectory_id != 0) {
//           continue;
...
//         // LOG(ERROR) << "init_pose_local_list " << init_pose_local_list.size();
//         // LOG(ERROR) << "nearby_submaps " << nearby_submaps.size();
//       }
//       // 创建submap数量的FastCorrelativeScanMatcher2D
//       int32_t submap_size = static_cast<int>(nearby_submaps.size());
//       absl::BlockingCounter created_counter{submap_size};
...
//         {
//             auto task = absl::make_unique<common::Task>();
//             task->SetWorkItem([this, &matchers, &created_counter, index, submap_id = id, &submaps, &submaps_vector, &submaps_vector_mutex] {
//                 try {
//                     const auto& submap_data = data_.submap_data.at(submap_id);
//                     if (!submap_data.submap) {
//                         LOG(ERROR) << "Submap is null for index " << index;
//                         throw std::runtime_error("Submap is null");
//                     }
//                     std::lock_guard<std::mutex> lock(submaps_vector_mutex);
//                     submaps[index] = static_cast<const Submap2D*>(submap_data.submap.get())->grid();
//                     submaps_vector.push_back(submaps[index]);                                              //999999999999999999999999999999999999
//                     matchers[index] = std::make_unique<scan_matching::FastCorrelativeScanMatcher2D>(
//                         *submaps[index],
//                         options_.constraint_builder_options().fast_correlative_scan_matcher_options());
//                     LOG(INFO) << "Task completed for index: " << index;
...
//         {
//             auto task = absl::make_unique<common::Task>();
//             task->SetWorkItem([i, &filtered_point_cloud, &matchers, &score_set, &pose_set, cutoff, &matched_counter, &has_matched, &task_durations, &timing_mutex, init_pose_local_list, &score_set_vector, &pose_set_vector, &results_mutex] {
//               //计时开始                                             
//               auto start_time = std::chrono::steady_clock::now();
...
//                 LOG(INFO) << "Processing2 matcher index: " << i;
//                 try {
//                   // LOG(ERROR) << "+++++++++++++++++++++++++++++++\n"<< "i:" << i << "init_pose_local_list[i]\n" << init_pose_local_list[i];
//                     if (matchers[i]->Match(init_pose_local_list[i], filtered_point_cloud, cutoff, &score, &pose_estimate)) 
//                     {
//                         std::lock_guard<std::mutex> lock(results_mutex);
//                         score_set_vector.push_back(score);
//                         pose_set_vector.push_back(pose_estimate);
//                         score_set[i] = score;
//                         pose_set[i] = pose_estimate;
//                         has_matched = true;
//                     } else {
...
//         search_submap_range *= ExpandRatio;
//         matched_submaps.insert(matched_submaps.end(),
//                        nearby_submaps.begin(),
//                        nearby_submaps.end());
//   }  //范围扩大搜索区域结束
...
//         LOG(ERROR) << "\n********************************************************\n"
//                   << "No matches found!\n"
//                   << "Given a new initial pose to retry\n";
//         return false;
//     }
...
//     if(score_set_vector.empty()){
//         LOG(ERROR) << "MATCH ALL FAILED! ";
//         return false;
//     }

//     int max_position = std::distance(score_set_vector.begin(), std::max_element(score_set_vector.begin(), score_set_vector.end()));
//     *best_score = score_set_vector[max_position];
//     *best_pose_estimate = pose_set_vector[max_position];


//     // CHECK(!score_set_vector.empty()) << "score_set_vector is empty!";
//     // //就尼玛的这里报错！！！！！！！！真坑：Check failed: !score_set_vector.empty() score_set_vector is empty!

//     // int max_position = std::distance(score_set_vector.begin(), std::max_element(score_set_vector.begin(), score_set_vector.end()));
//     // CHECK_LT(max_position, score_set_vector.size());
//     // CHECK_LT(max_position, pose_set_vector.size());
//     // CHECK_LT(max_position, submaps_vector.size());
//     // *best_score = score_set_vector[max_position];
//     // *best_pose_estimate = pose_set_vector[max_position];
...
//     try {
//         csm->Match(best_pose_estimate->translation(), *best_pose_estimate,
//                    filtered_point_cloud, *submaps_vector[max_position],
//                    best_pose_estimate, &unused_summary);
//     } catch (const std::exception& e) {
//         LOG(ERROR) << "CeresScanMatcher2D failed: " << e.what();
//         return false;
...
//             << "[GlobalPositioningTest] best_score:" <<  *best_score << "\n"
//             << "The distance between best_pose_estimate and Given_initial_pose:"
//             << (pose_set_vector[max_position].translation() - Given_initial_pose.translation().head<2>()).norm()
//             << "\n---------------------------------------------------\n";
//   return true;
//   }
